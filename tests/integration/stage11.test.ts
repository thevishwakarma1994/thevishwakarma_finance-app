...
async function setup(openingPaise = 400_000) {
...
async function avail(handles: SqliteHandles, workspaceId: string, accountId: string) {
  return accountAvailability(await loadSnapshot(handles, workspaceId), accountId);
}

async function cardShare(ctx: Awaited<ReturnType<typeof setup>>, amountPaise: number, rahulPaise: number, userPaise: number) {
  return await recordSplit(ctx.handles, { workspaceId: ctx.workspaceId }, {
...
async function collect(ctx: Awaited<ReturnType<typeof setup>>, claimId: string, amountPaise: number, allocatedPaise = amountPaise) {
  return await receiveSettlement(ctx.handles, { workspaceId: ctx.workspaceId }, {
    occurredOn: "2026-08-16",
    capturedAt,
...
describe("stage 11 reservations and surplus", () => {
  ...
  it("A — card collection reservation", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    ...
    const availableBefore = (await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).availablePaise;
    await collect(ctx, claim.id, 250_000);
    const after = await loadSnapshot(ctx.handles, ctx.workspaceId);
    const hdfc = await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId);
  ...
  it("B — partial collection", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 100_000);
    expect((await loadSnapshot(ctx.handles, ctx.workspaceId)).reservations[0]?.remainingPaise).toBe(100_000);
  });

  it("C — mixed settlement", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    ...
    const hdfc = await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId);
    expect((await loadSnapshot(ctx.handles, ctx.workspaceId)).reservations[0]?.remainingPaise).toBe(250_000);
    expect(hdfc.availablePaise).toBe(availableBefore + 150_000);
  });

  it("D — card already paid", async () => {
    ...
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).availablePaise).toBe(availableBefore + 250_000);
  });

  it("E — ordinary spend protection", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 800_000, 600_000, 200_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 600_000);
    const hdfc = await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId);
  ...
  it("F — transfer protection", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 800_000, 600_000, 200_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
  ...
  it("G — same-account card payment consumes reservation first", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 800_000, 600_000, 200_000);
    const before = await loadSnapshot(ctx.handles, ctx.workspaceId);
    ...
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).availablePaise).toBe(200_000);
    expect(after.billingCycles[0]?.ledgerRemainingPaise).toBe(0);
  });

  it("H — different-account full payment releases reservation", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 600_000, 600_000, 0);
    const before = await loadSnapshot(ctx.handles, ctx.workspaceId);
    const claim = before.claims[0];
    const cycle = before.billingCycles[0];
    if (!claim || !cycle) throw new Error("missing");
    ...
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).availablePaise).toBe(availableBefore + 600_000);
  });

  it("I — different-account partial payment releases only what is no longer required", async () => {
    const ctx = await setup(400_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 600_000, 600_000, 0);
    const before = await loadSnapshot(ctx.handles, ctx.workspaceId);
  ...
  it("J — incoming overpayment creates pending surplus", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 300_000, 250_000);
    const after = await loadSnapshot(ctx.handles, ctx.workspaceId);
    const hdfc = await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId);
  ...
  it("K — claim allocation cannot exceed open amount", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await expect(collect(ctx, claim.id, 250_000, 300_000)).rejects.toThrow(DomainError);
  });

  it("L — apply surplus to another claim", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 250_000, 250_000, 0);
    await cardShare(ctx, 150_000, 150_000, 0);
    const claimsBefore = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims;
    const first = claimsBefore[0];
    const second = claimsBefore[1];
    if (!first || !second) throw new Error("missing claims");
    await collect(ctx, first.id, 300_000, 250_000);
    const surplus = (await loadSnapshot(ctx.handles, ctx.workspaceId)).surplusCases[0];
    if (!surplus) throw new Error("missing surplus");
    await resolveSurplus(ctx.handles, { workspaceId: ctx.workspaceId }, {
  ...
  it("M — convert surplus to payable", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
    ...
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).pendingSurplusHeldPaise).toBe(0);
  });

  it("N — treat-as-mine", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
    ...
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 300_000, 250_000);
    const before = await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId);
    const surplus = (await loadSnapshot(ctx.handles, ctx.workspaceId)).surplusCases[0];
    if (!surplus) throw new Error("missing surplus");
    await expect(
    ...
    const after = await loadSnapshot(ctx.handles, ctx.workspaceId);
    expect(after.surplusCases[0]?.status).toBe("resolved");
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).availablePaise).toBe(before.availablePaise + 50_000);
    expect(after.postings.some((posting) => posting.pnl === "income_other" || posting.pnl === "income_salary")).toBe(false);
  });

  it("O — reservation reassignment", async () => {
    const ctx = await setup(1_000_000);
    contexts.push(ctx.handles);
    const axis = await createCard(ctx.handles, { workspaceId: ctx.workspaceId }, {
      displayName: "AXIS",
      issuer: "AXIS",
      mask: "4412",
      statementDay: 12,
    ...
    const iciciCycle = (await loadSnapshot(ctx.handles, ctx.workspaceId)).billingCycles.find(
      (cycle) => cycle.creditCardId === ctx.cardId,
    );
    const axisCycle = (await loadSnapshot(ctx.handles, ctx.workspaceId)).billingCycles.find(
    ...
    const surplus = (await loadSnapshot(ctx.handles, ctx.workspaceId)).surplusCases.find(
    ...
    expect((await avail(ctx.handles, ctx.workspaceId, ctx.hdfcId)).balancePaise).toBe(
      (await loadSnapshot(ctx.handles, ctx.workspaceId)).accounts.find((account) => account.id === ctx.hdfcId)?.balancePaise,
    );
  });

  it("P — pending surplus and reserved are not double-counted", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
  ...
  it("Q — person net is unaffected by reservation itself", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 250_000);
    const position = personPosition((await loadSnapshot(ctx.handles, ctx.workspaceId)).claims, ctx.rahulId);
    expect(position.netPaise).toBe(0);
    expect((await loadSnapshot(ctx.handles, ctx.workspaceId)).reservations[0]?.remainingPaise).toBe(250_000);
    expect((await personDetail(ctx.handles, ctx.workspaceId, ctx.rahulId)).netPaise).toBe(0);
  });

  it("R — Month Review unaffected by reservation/surplus", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const before = await monthReview(ctx.handles, ctx.workspaceId, isoDate("2026-08-16"));
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await collect(ctx, claim.id, 300_000, 250_000);
    const after = await monthReview(ctx.handles, ctx.workspaceId, isoDate("2026-08-16"));
    expect(after.spentPaise).toBe(before.spentPaise);
    expect(after.spentPaise).toBe(150_000);
  });

  it("S — invalid reservation/surplus transition is atomic", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
...
describe("stage 11 read models", () => {
  ...
  it("exposes reserved/available and settlement consequences", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await cardShare(ctx, 400_000, 250_000, 150_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    const cycle = (await loadSnapshot(ctx.handles, ctx.workspaceId)).billingCycles[0];
    if (!claim || !cycle) throw new Error("missing");
    await collect(ctx, claim.id, 250_000);
    const accounts = await listAccounts(ctx.handles, ctx.workspaceId);
    ...
    const activity = (await listActivity(ctx.handles, ctx.workspaceId)).find((event) => event.meaning === "settlement_in");
    expect(activity?.consequences?.some((item) => item.kind === "reserved")).toBe(true);
    const detail = await cycleDetail(ctx.handles, ctx.workspaceId, cycle.id);
...
describe("stage 11 surplus resolution event identity", () => {
  ...
  it("A — surplus on date A resolved on date B has a distinct resolution event", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
    ...
    const originPostingsBefore = ctx.handles.db
      ...
      .all();
    const resolved = await resolveSurplus(ctx.handles, { workspaceId: ctx.workspaceId }, {
      surplusCaseId: surplus.id,
      resolution: "treat_as_mine_correction",
      confirmed: true,
  ...
  it("B — reservation reassignment ledger references the resolution event", async () => {
    const ctx = await setup(1_000_000);
    contexts.push(ctx.handles);
    const axis = await createCard(ctx.handles, { workspaceId: ctx.workspaceId }, {
      displayName: "AXIS",
  ...
  it("C — convert to payable remains traceable to the resolution event", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
  ...
  it("D — treat-as-mine has a distinct resolution event and no P&L", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
  ...
  it("E — failed resolution writes no resolution event or ledger mutation", async () => {
    const ctx = await setup(1_000_000);
    contexts.push(ctx.handles);
    await cardShare(ctx, 250_000, 250_000, 0);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
