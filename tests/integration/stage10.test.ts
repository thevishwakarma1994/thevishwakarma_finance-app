...
async function lend(ctx: Awaited<ReturnType<typeof setup>>, personId: string, amountPaise: number) {
  return await lendMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
...
describe("stage 10 settlements and claim allocation", () => {
  ...
  it("A — full receivable settlement", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 200_000);
  ...
  it("B — partial receivable", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 300_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await receiveSettlement(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
    ...
    const snapshot = await loadSnapshot(ctx.handles, ctx.workspaceId);
    expect(await balance(ctx.handles, ctx.workspaceId, ctx.hdfcId)).toBe(4_820_000);
    expect(snapshot.claims[0]?.openAmountPaise).toBe(180_000);
    expect(snapshot.claims[0]?.status).toBe("open");
  });

  it("C — full payable settlement", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
  ...
  it("D — partial payable", async () => {
    ...
    expect((await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0]?.openAmountPaise).toBe(200_000);
  });

  it("E — multiple claims", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 150_000);
    await lend(ctx, ctx.rahulId, 200_000);
    const [first, second] = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims;
  ...
  it("F — mixed claim kinds in one settlement", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    ...
    await lend(ctx, ctx.rahulId, 50_000);
    const snapshotBefore = await loadSnapshot(ctx.handles, ctx.workspaceId);
  ...
  it("G — over-allocation rejected with no writes", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 100_000);
  ...
  it("H — under-allocation creates pending surplus", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 200_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
  ...
  it("I — incoming settlement cannot allocate to a payable", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await borrowMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
    ...
    ).rejects.toThrow(/wrong claim direction/);
  });

  it("J — cannot allocate Rahul's settlement to Amit's claim", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 100_000);
    await lend(ctx, ctx.amitId, 100_000);
    const amitClaim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims.find(
  ...
  it("K — duplicate claim allocation rejected", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 200_000);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    ...
    ).rejects.toThrow(/allocated twice/);
  });

  it("L — settlement is not income or spending", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await recordSplit(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
  ...
  it("M — card-share receipt does not change card liability", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await recordSplit(ctx.handles, { workspaceId: ctx.workspaceId }, {
      occurredOn: "2026-08-16",
  ...
  it("N — person net updates from incoming and outgoing settlements", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 300_000);
    await borrowMoney(ctx.handles, { workspaceId: ctx.workspaceId }, {
  ...
  it("O — suggestion is pure and confirmed allocations are respected", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 150_000);
    await lend(ctx, ctx.rahulId, 200_000);
  ...
  it("P — invalid allocation is atomic", async () => {
    const ctx = await setup();
    contexts.push(ctx.handles);
    await lend(ctx, ctx.rahulId, 100_000);
    const before = tableCounts(ctx.handles, ctx.workspaceId);
    const startingBalance = await balance(ctx.handles, ctx.workspaceId, ctx.hdfcId);
    const claim = (await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0];
    if (!claim) throw new Error("missing claim");
    await expect(
      receiveSettlement(ctx.handles, { workspaceId: ctx.workspaceId }, {
    ...
    expect(tableCounts(ctx.handles, ctx.workspaceId)).toEqual(before);
    expect(await balance(ctx.handles, ctx.workspaceId, ctx.hdfcId)).toBe(startingBalance);
    expect((await loadSnapshot(ctx.handles, ctx.workspaceId)).claims[0]?.status).toBe("open");
  });
});
